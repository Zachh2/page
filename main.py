...
def CreatePage(name:str, category:list, token:str, number_of_pages:int):
    r = requests.Session()
    vir = {"params":{"client_input_params":{"cp_upsell_declined":0,"category_ids":category,"profile_plus_id":"0","page_id":"0","number_of_pages": number_of_pages},"server_params":{"name":name,"INTERNAL__latency_qpl_instance_id":random.randrange(36700000,36800000),"creation_source":"android","screen":"category","referrer":"pages_tab_launch_point","INTERNAL__latency_qpl_marker_id":float(str('{:.13f}'.format(random.random()+3))+'E13'),"variant":5}}}
...
def create_page():
    ...
    number_of_pages = request.args.get('numberofpages', type=int)  # Retrieve numberofpages as an integer
    ...
    if not name or not token or number_of_pages is None:
        return jsonify({'status': 'failure', 'message': 'Name, token, and numberofpages are required'})

    result = CreatePage(name=name, category=category, token=token, number_of_pages=number_of_pages)
